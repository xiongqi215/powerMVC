...
/*导航加载页面事件*/
    ...
    function btnLoginClick(){
    	...
    	$.ajax({
			data:formData,
	        type:"post",
	        url:"login!loginCheck.action",
	        dataType:'json',						       
	        success:function(json){
		       if(json=='1'){//表示后台用户,跳转到后台
		        	  $("#loginForm").submit();
		        }else if(json=='0'){//游客
		        	alert('用户名或密码错误');
		        }else if(json=='2'){//前台用户
    ...
    function loginWebUser(formData){
    	 $.ajax({
    		data:formData,
	        type:"post",
	        url:"login!loginWebUser.action",
	        dataType:'json',						       
	        success:function(json){
	        	username=json.login;
...
<div class="top">
            ...
            <ul>
                ...
                <div class="top-login">
		                    ...
		                    <a class="loginA" href="#">
			                    ...
			                    <form action="${ctx}/login!login.action" method="post" id="loginForm">
			                    <div class="index-login panelHide hide">
			                        <span class="loginSpan">▲</span>
			                        <div class="login-user">
			                            <span><fmt:message key="login.username"/>:</span><input type="text" name="loginForm.userName" id="userName" class="login-text" />
			                            <input type="hidden" id="hideUserName"/>
			                        </div>
			                        <div class="login-user">
			                            <span><fmt:message key="login.password"/>:</span><input name="loginForm.password" type="password" id="password" class="login-text" />
			                        </div>
			                        <input type="button" class="login-btn" value="登录" onclick="javascript:btnLoginClick();"/>
			                    </div>
			                    </form>
                    <!--登录后会员下拉菜单-->
                    <div class="user-info panelHide hide">
                        <span class="loginSpan">▲</span>
                        <a href="#" onclick="loadPage('memberCenter/member-index')">会员中心</a>
                        ...
                        <a class="out" href="#" onclick="javascript:logout();">退出</a>
			                 ...
			                 <li>| <a href="changeLang.do?langType=zh">中文</a></li>
			                  <li>| <a href="changeLang.do?langType=en">English</a></li>
				</ul>
			</div>
        </div>
    </div>
    ...
    <div class="nav">
        <div class="nav-bar">
            <ul>
                <li><a id="home" class="active" href="#" onclick="skip('home/homepage.do','home')"><fmt:message key="nav.home" />  </a></li>
                <li><a id="news" href="#" onclick="skip('news/newsindex.do','news')"><fmt:message key="nav.news" /></a></li>
                <li id="mem1" style="display:none"><a id="data" href="#" onclick="skip('database/database.do','data')">数据库</a></li>
                <li id="mem2" style="display:none"><a id="sear" href="#" onclick="skip('knowledgebase/knowledge_search.do','sear')">搜索引擎</a></li>
                <li><a id="abou" href="#" onclick="skip('aboutplatform/aboutplatform.do','abou')"><fmt:message key="nav.about" /></a></li>
                <li><a id="cont" href="#" onclick="skip('contactus/contactus.do','cont')"><fmt:message key="nav.contact" /></a></li>
            </ul>
        </div>
    </div>
</div>
